...
Dataset substitution for GSM8K:
  By default, GSM8K experiments use the standard GSM8K dataset.
  
  To substitute with the AI-MO dataset ("AI-MO/aimo-validation-aime"), set:
      export USE_AIMO_DATASET=1
  before running this script. The sitecustomize patch (experiments/patch/) will
  intercept all load_dataset("gsm8k") calls and redirect them to AI-MO.
  
  Convenience script:
      python scripts/run_aimo_gsm8k.py
  This automatically sets USE_AIMO_DATASET=1 and runs only GSM8K experiments.
  
  The PYTHONPATH for sitecustomize is automatically injected into srun payloads,
  so no manual setup is required when running through this script.
"""
...
def get_task_total(task: str) -> int:
    ...
    For GSM8K, returns 90 if USE_AIMO_DATASET is set, otherwise 1319.
    """
    import os
...
def resolve_llada_eval_script() -> Path:
    return _resolve_existing_path(
        [
            ROOT_DIR / "llada" / "eval_llada.py",
            ROOT_DIR / "v1" / "llada" / "eval_llada.py",
        ],
        "LLaDA eval script",
    )


def resolve_dream_eval_script() -> Path:
    return _resolve_existing_path(
        [
            ROOT_DIR / "dream" / "eval.py",
            ROOT_DIR / "v1" / "dream" / "eval.py",
        ],
        "Dream eval script",
    )
...
def build_payload(run: ExperimentRun, save_dir: Path, output_path: Path) -> str:
    ...
    if isinstance(model, LLaDaConfig):
        ...
        work_dir = ROOT_DIR
        dream_eval_script = resolve_dream_eval_script()
        model_args = _dream_model_args(model, save_dir)
        fewshot_flag = f"--num_fewshot {run.num_fewshot} " if run.num_fewshot is not None else ""
        eval_cmd = (
            f"accelerate launch {dream_eval_script} "
            f"--model dream "
            f"--model_args '{model_args}' "
            f"--tasks {run.task} "
            f"{fewshot_flag}"
    ...
    env_exports = [
        f"source {run.venv}",
        f"export PYTHONPATH={ROOT_DIR / 'experiments' / 'patch'}:${{PYTHONPATH:-}}",
    ...
    # Conditionally pass through USE_AIMO_DATASET if set in parent environment
    if os.environ.get("USE_AIMO_DATASET", "").lower() in ("1", "true", "yes"):
        env_exports.append("export USE_AIMO_DATASET=1")
    
    return (
        " && ".join(env_exports) +
        f" && cd {work_dir} && {eval_cmd}"
    )
...
def run_experiment(run: ExperimentRun) -> None:
    ...
    total = get_task_total(run.task)
    if total is None:
        raise ValueError(f"Unknown task '{run.task}'. Add it to get_task_total().")
...
EXPERIMENTS: list[ExperimentRun] = [
    ...
    *[
    ...
    # ── LLaDA GSM8K-AIMO (AI-MO dataset) — 5-shot ──────────────────────────
    ExperimentRun(
        tag="baseline",
        experiment_name="GSM8K-LLaDA-AIMO",
        task="gsm8k",
        num_fewshot=5,
        model=LLaDaConfig(gen_length=512, steps=512, block_length=32),
    ),
    ...
    # ── Dream GSM8K-AIMO (AI-MO dataset) — 5-shot ──────────────────────────
    # ExperimentRun(
    #     tag="baseline",
    #     experiment_name="GSM8K-Dream-AIMO",
    #     task="gsm8k",
    #     num_fewshot=5,
    #     model=DreamConfig(max_new_tokens=256, diffusion_steps=256, alg="entropy"),
    # ),
